...
class _BaseTrendForecaster(BasePointForecaster):
    ...
    def _prepare_data(self, y: pl.DataFrame, panel_group_name: str | None = None) -> tuple:
        ...
        if panel_group_name is not None:
            y_array = (
                y[panel_group_name].select([col for col in y[panel_group_name].columns if col != "time"]).to_numpy()
            )
        else:
    ...
    def _fit_estimator(self, estimator: RegressorMixin, y_t: pl.DataFrame | dict[str, pl.DataFrame]) -> None:
        # Non-panel data
