...
def check_search_fit_sets_attributes(
    ...
    if search_cv_clone.refit:
        assert hasattr(search_cv_clone, "best_forecaster_"), (
            "fit() must set best_forecaster_ when refit=True"
        )
...
def check_search_not_fitted_error(search_cv, y: pl.DataFrame, X: pl.DataFrame | None = None) -> None:
    ...
    try:
        ...
        raise AssertionError(
            f"{search_cv_clone.__class__.__name__} should raise NotFittedError "
            f"when calling predict() before fit()"
        )
...
def check_search_cv_results_structure(
    ...
    if not search_cv_clone.multimetric_:
        ...
        assert len(cv_results["mean_test_score"]) == n_candidates, (
            f"mean_test_score length {len(cv_results['mean_test_score'])} "
            f"should match params length {n_candidates}"
        )
        assert len(cv_results["rank_test_score"]) == n_candidates, (
            f"rank_test_score length {len(cv_results['rank_test_score'])} "
            f"should match params length {n_candidates}"
        )
    ...
    if not search_cv_clone.multimetric_:
        ...
        for split_idx in range(n_splits):
            ...
            assert len(cv_results[split_key]) == n_candidates, (
                f"{split_key} length {len(cv_results[split_key])} "
                f"should match params length {n_candidates}"
            )
    else:
...
def check_search_refit_false_no_forecaster(
    ...
    assert hasattr(search_cv_clone, "best_params_"), (
        "fit() should set best_params_ even when refit=False"
    )
    assert hasattr(search_cv_clone, "best_score_"), (
        "fit() should set best_score_ even when refit=False"
    )
    assert not hasattr(search_cv_clone, "best_forecaster_"), (
        "fit() should NOT set best_forecaster_ when refit=False"
    )
    assert not hasattr(search_cv_clone, "refit_time_"), (
        "fit() should NOT set refit_time_ when refit=False"
    )
    ...
    try:
        search_cv_clone.predict(forecasting_horizon=1, X=X)
        raise AssertionError(
            f"{search_cv_clone.__class__.__name__} should not have predict() when refit=False"
        )
...
def check_search_predict_delegates(
    search_cv, y_train: pl.DataFrame, y_test: pl.DataFrame,
    X_train: pl.DataFrame | None = None, X_test: pl.DataFrame | None = None
) -> None:
...
def check_search_update_delegates(
    search_cv, y_train: pl.DataFrame, y_update: pl.DataFrame,
    X_train: pl.DataFrame | None = None, X_update: pl.DataFrame | None = None
) -> None:
    """Check update() delegates to best_forecaster_.update() correctly.
    ...
    ------
    AssertionError
        If update() doesn't delegate correctly

    """
    # Get initial observed_time
    initial_observed_time = search_cv.best_forecaster_.observed_time_

    # Update via search CV
    search_cv.update(y_update, X_update)

    # Check that best_forecaster_ was updated
    ...
    if isinstance(initial_observed_time, dict):
        ...
        for group_name in initial_observed_time:
            assert updated_observed_time[group_name] > initial_observed_time[group_name], (
                f"observed_time for group {group_name} should increase after update, "
        ...
        assert updated_observed_time > initial_observed_time, (
            f"observed_time should increase after update, "
            f"got {initial_observed_time} -> {updated_observed_time}"
        )


def check_search_reset_delegates(
    search_cv, y_train: pl.DataFrame, y_reset: pl.DataFrame,
    X_train: pl.DataFrame | None = None, X_reset: pl.DataFrame | None = None
) -> None:
    """Check reset() delegates to best_forecaster_.reset() correctly.
    ...
    ------
    AssertionError
        If reset() doesn't delegate correctly
    ...
    # Reset via search CV
    search_cv.reset(y_reset, X_reset)

    # Check that best_forecaster_ was reset
    ...
    if isinstance(initial_observed_time, dict):
        ...
        for group_name in initial_observed_time:
            assert reset_observed_time[group_name] != initial_observed_time[group_name], (
                f"observed_time for group {group_name} should change after reset"
            )
    else:
        # Non-panel case
        assert reset_observed_time != initial_observed_time, (
            f"observed_time should change after reset"
        )
...
def check_search_score_delegates(
    ...
    if search_cv.multimetric_:
        assert isinstance(score, dict), (
            f"score() should return dict for multimetric, got {type(score)}"
        )
...
def check_search_multimetric_scoring(
    ...
    if isinstance(search_cv_clone.refit, str):
        ...
        expected_best_score = search_cv_clone.cv_results_[f"mean_test_{refit_metric}"][
            search_cv_clone.best_index_
        ]
...
def check_search_return_train_score(
    ...
    if not search_cv_clone.multimetric_:
        assert "mean_train_score" in cv_results, (
            "cv_results_ must have 'mean_train_score' when return_train_score=True"
        )
        ...
        for split_idx in range(n_splits):
            ...
            assert split_key in cv_results, (
                f"cv_results_ must have '{split_key}' when return_train_score=True"
            )
...
def check_search_clone_preserves_params(search_cv) -> None:
    ...
    assert type(params_original["forecaster"]) == type(params_cloned["forecaster"]), (
    ...
    if params_original["scoring"] is not None:
        if isinstance(params_original["scoring"], dict):
            assert isinstance(params_cloned["scoring"], dict), (
                "Cloned scoring should be dict when original is dict"
            )
        else:
            assert type(params_original["scoring"]) == type(params_cloned["scoring"]), (
    ...
    if hasattr(search_cv, "cv_results_"):
        assert not hasattr(search_cv_cloned, "cv_results_"), (
            "Cloned search CV should not have fitted attributes"
        )
...
def check_grid_search_exhaustive(
    ...
    assert actual_combinations == expected_combinations, (
        f"GridSearchCV should evaluate {expected_combinations} combinations, "
    ...
    if isinstance(param_grid, dict):
        ...
        for key, value in param_grid.items():
            ...
            assert isinstance(value, (list, tuple)), (
                f"param_grid values should be list or tuple, got {type(value)}"
            )
    elif isinstance(param_grid, list):
        # Valid: list of dicts
        for grid in param_grid:
            assert isinstance(grid, dict), (
                f"param_grid list elements should be dict, got {type(grid)}"
            )
        ...
        raise AssertionError(
            f"param_grid should be dict or list of dicts, got {type(param_grid)}"
        )
...
def check_randomized_search_n_iter(
    ...
    assert actual_combinations == expected_combinations, (
        f"RandomizedSearchCV should evaluate {expected_combinations} combinations (n_iter), "
        f"got {actual_combinations}"
    )
...
def check_randomized_search_reproducibility(
    ...
    assert params1 == params2, (
        "RandomizedSearchCV should produce same parameters with same random_state"
    )
