...
try:
    ...
    raise ImportError(
        "polars is required for yohou.testing module. Install with: uv sync --group tests"
    ) from e
...
def check_splitter_tags_accessible_before_fit(splitter) -> None:
    ...
    assert hasattr(splitter, "__sklearn_tags__"), (
        f"{splitter_name} must have __sklearn_tags__() method"
    )
    ...
    try:
        ...
        raise AssertionError(
            f"{splitter_name}.__sklearn_tags__() raised {type(e).__name__}: {e}"
        ) from e
...
def check_splitter_tags_static_after_fit(
    splitter, y: pl.DataFrame, X: pl.DataFrame | None = None
) -> None:
...
def check_splitter_tags_match_capabilities(
    ...
    expected_tags : dict, optional
        Expected tag values to validate (keys: splitter_type, requires_data_for_n_splits, etc.)
    ...
    if expected_tags is not None:
        for key, expected_value in expected_tags.items():
            ...
            assert actual_value == expected_value, (
                f"Tag mismatch: {key}={actual_value}, expected {expected_value}"
            )

    # Validate requires_data_for_n_splits
    if splitter_tags.requires_data_for_n_splits:
        # Should raise ValueError if y is None
        try:
            splitter.get_n_splits(y=None, X=None)
            raise AssertionError(
                "Splitter declares requires_data_for_n_splits=True but "
                "get_n_splits(y=None) did not raise ValueError"
            )
        except ValueError:
            pass  # Expected
    else:
        ...
        try:
            ...
            raise AssertionError(
                f"Splitter declares requires_data_for_n_splits=False but "
                f"get_n_splits(y=None) raised ValueError: {e}"
            ) from e
...
def check_splitter_produces_valid_indices(
    splitter, y: pl.DataFrame, X: pl.DataFrame | None = None
) -> None:
    ...
    for i, (train_idx, test_idx) in enumerate(splitter.split(y, X)):
        ...
        assert np.all(train_idx < n_samples), (
            f"Split {i}: train indices out of bounds (>= {n_samples})"
        )
        ...
        assert np.all(test_idx < n_samples), (
            f"Split {i}: test indices out of bounds (>= {n_samples})"
        )
...
def check_splitter_n_splits_consistency(
    splitter, y: pl.DataFrame, X: pl.DataFrame | None = None
) -> None:
...
def check_splitter_non_overlapping_tests(
    splitter, y: pl.DataFrame, X: pl.DataFrame | None = None
) -> None:
...
def check_splitter_panel_data_support(
    ...
    try:
        ...
        raise AssertionError(
            f"Splitter declares supports_panel_data=True but failed on panel data: {e}"
        ) from e
...
def check_splitter_parameter_constraints(
    ...
    if splitter_class == SlidingWindowSplitter:
        defaults = {"train_size": 10, "test_size": 5}
    ...
    for invalid_value in invalid_values:
        try:
            ...
            try:
                # Try get_n_splits if it doesn't require data
                splitter.get_n_splits(y=None, X=None)
            except ValueError as e:
                # Check if it's a parameter validation error (not a "y required" error)
                error_msg = str(e)
                if param_name in error_msg or "parameter" in error_msg.lower():
                    continue  # Parameter validation worked
                # Otherwise it's a different ValueError (like y required)
            ...
            raise AssertionError(
                f"{splitter_class.__name__}: invalid {param_name}={invalid_value} was accepted"
            )
