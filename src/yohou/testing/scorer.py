...
def check_scorer_tags_accessible_before_fit(scorer) -> None:
    ...
    try:
        ...
        raise AssertionError(
            f"{scorer_name}.__sklearn_tags__() raised {type(e).__name__}: {e}"
        ) from e
...
def check_scorer_tags_match_capabilities(
    ...
    if scorer_tags.prediction_type == "point":
        ...
        assert not interval_cols, (
            f"Point scorer received interval predictions with columns: {interval_cols}"
        )
...
def check_scorer_prediction_type_compatibility(
    ...
    try:
        ...
        assert isinstance(score, (int, float, np.number)), (
            f"score() should return numeric value, got {type(score)}"
        )
    except Exception as e:
...
def check_scorer_lower_is_better(scorer) -> None:
    ...
    # Just check that the tag exists and is boolean
    assert isinstance(lower_is_better, bool), (
        f"lower_is_better should be bool, got {type(lower_is_better)}"
    )
    ...
    for agg_method in aggregation_methods:
        ...
        try:
            ...
            if isinstance(score, pl.DataFrame):
                ...
                raise AssertionError(
                    f"aggregation_method={agg_method}: score should be numeric or DataFrame, "
                    f"got {type(score)}"
                )
        except Exception as e:
...
def check_scorer_component_subselection(
    ...
    try:
        ...
        assert isinstance(score, (int, float, np.number)), (
            "Component-filtered score should be numeric"
        )
    except Exception as e:
...
def check_scorer_coverage_rate_subselection(
    ...
    try:
        ...
        assert isinstance(score, (int, float, np.number)), (
            "Coverage-filtered score should be numeric"
        )
    except Exception as e:
...
def check_scorer_parameter_validation(
    ...
    try:
        scorer.fit(y_truth)
        raise AssertionError(
            f"{scorer_class.__name__}: invalid {param_name}={invalid_value} was accepted in fit()"
        )
...
def check_scorer_methods_call_check_is_fitted(
    scorer, y_train: pl.DataFrame, y_pred: pl.DataFrame
) -> None:
