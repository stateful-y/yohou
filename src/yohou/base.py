...
class BaseForecaster(BaseEstimator, metaclass=abc.ABCMeta):
    ...
    def _update_y_X_t_observed(self, y: pl.DataFrame, X_t: pl.DataFrame | None, panel_group_names: list[str]) -> None:
    ...
    def _pre_fit(
        ...
        if self.panel_group_names_ is not None:
            y = {
                group: y.select(["time"] + [f"{group}__{col}" for col in self.local_y_schema_.keys()]).rename({
                    f"{group}__{col}": col for col in self.local_y_schema_.keys()
                })
        ...
        # Panel data
            ...
            for panel_group_name in panel_group_names:
                ...
                if self._y_observed is not None and panel_group_name in self._y_observed:
                    y_full = pl.concat([self._y_observed[panel_group_name], y_local], how="vertical")
                else:
                    y_full = y_local
    ...
    def _add_time_columns(self, y_pred: pl.DataFrame) -> pl.DataFrame:
        ...
        if self.panel_group_names_ is not None:
            ...
            predicted_times = [
                add_interval(observed_time_value, self.interval_, n=n) for n in range(1, len(y_pred) + 1)
            ]
        ...
        # Global data case: observed_time_ is a datetime
            ...
            predicted_times = [
                add_interval(self.observed_time_, self.interval_, n=n) for n in range(1, len(y_pred) + 1)
            ]
    ...
    def _predict(
        ...
        **predict_one_params : dict
        ...
        y_pred_step = forecaster._predict_one(panel_group_names=panel_group_names, **predict_one_params)
        ...
        if forecaster.target_transformer is None:
        ...
        elif panel_group_names is None:
            # Non-panel data
            ...
            y_pred_step_inv_cast = cast(y_pred_step_inv.select(~cs.by_name("time")), forecaster.local_y_schema_)
            ...
            for panel_group_name in panel_group_names:
                ...
                local_y_schema = {
                    f"{panel_group_name}__{col}": dtype for col, dtype in forecaster.local_y_schema_.items()
                }
...
class BaseWrapper(BaseEstimator, metaclass=abc.ABCMeta):
    ...
    **params
    ...
    def _validate_estimator_params(self, params: dict[str, object]) -> dict[str, object]:
        ...
        valid_class_params = {
            key: val.default for key, val in constructor_signature.parameters.items() if key != "self"
        }
...
def _build_feature_input(
    ...
    if input_features == "y_t|X":
        ...
        raise ValueError(f"Invalid input_features='{input_features}'. Must be one of: 'y_t|X', 'y|X', 'X'.")
...
def _reset_transformers_one(
    ...
    X_feat_in = _build_feature_input(y_for_feat, y_t, X_for_feat, input_features, feature_transformer)
