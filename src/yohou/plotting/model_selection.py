    ...
    for i, (train_idx, test_idx) in enumerate(splits):
        ...
        fig.add_trace(
            go.Scatter(
                ...
                legendgroup="train",
                customdata=[
                    [t_train_start, t_train_end],
                    [t_train_start, t_train_end],
                ],
                hovertemplate=(
                    "Train<br>Start: %{customdata[0]}<br>"
                    f"End: %{{customdata[1]}}<br>Fold: {fold_label}<extra></extra>"
                ),
            )
        )
        ...
        fig.add_trace(
            go.Scatter(
                ...
                hovertemplate=(
                    f"Test<br>Start: %{{customdata[0]}}<br>End: %{{customdata[1]}}<br>Fold: {fold_label}<extra></extra>"
                ),
            )
        )
