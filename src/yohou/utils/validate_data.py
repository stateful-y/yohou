...
def validate_time_weight(
    ...
    if panel_group_names is None:
        ...
        if "weight" not in time_weight.columns:
            raise ValueError(
                "time_weight DataFrame for global data must have 'weight' column"
            )
...
def validate_scorer_data(
    ...
    *,
    ...
    if inverse:
        ...
        y_pred_cols = set(
            y_pred.select(~cs.by_name(*exclude_cols_pred)).columns
        )
        score_cols = set(
            scores.select(~cs.by_name(*exclude_cols_scores)).columns
        )
    ...
    if set(y_groups.keys()) != set(X_groups.keys()):
        raise ValueError(
            f"Panel groups mismatch. `y_true` has {sorted(y_groups.keys())}. "
            f"`y_pred` has {sorted(X_groups.keys())}."
        )
    ...
    for col in y_true.columns:
        ...
        if pred_type == "point":
            ...
            if not lower_found or not upper_found:
                raise ValueError(
                    f"Interval columns for `y_true` '{col}' missing in `y_pred`."
                )
    ...
    *,
    ...
    if X is not None:
        ...
        if forecaster.panel_group_names_ is not None:
            ...
            if X is not None and hasattr(forecaster, "local_X_schema_") and forecaster.local_X_schema_:
    ...
    **check_params,
    ...
    if inverse:
        ...
        if X_t is None:
            if X is None:
                raise ValueError(
                    "Either `X_t` or `X` must be provided for inverse transform."
                )
            X_t = X
