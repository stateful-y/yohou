...
class ColumnForecaster(BaseForecaster, _BaseComposition):
    ...
    def __sklearn_tags__(self) -> Tags:
        ...
        if hasattr(self, "forecasters_") and self.forecasters_:
            ...
            tags.forecaster_tags.uses_reduction = any(
                getattr(f.__sklearn_tags__().forecaster_tags, "uses_reduction", False) for f in self.forecasters_
            )
            ...
            tags.forecaster_tags.supports_panel_data = all(
                getattr(f.__sklearn_tags__().forecaster_tags, "supports_panel_data", True) for f in self.forecasters_
            )
    ...
    def predict(
        ...
        **params : dict
        ...
        predictions = Parallel(n_jobs=self.n_jobs)(
            ...
            for forecaster, (name, _, _) in zip(self.forecasters_, self.forecasters, strict=False)
        )
    ...
    def update(
        ...
        Parallel(n_jobs=self.n_jobs)(
            ...
            # TODO: What's this?
            for forecaster, (_, _, cols) in zip(self.forecasters_, self.forecasters, strict=False)
        )
