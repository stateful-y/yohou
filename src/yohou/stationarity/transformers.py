    ...
    def fit(self, X: pl.DataFrame, y: pl.DataFrame | None = None, **params) -> "ASinhTransformer":
        ...
        **params : dict
        ...
        for col in X_numeric.columns:
            ...
            self.median_[col] = float(median_val) if median_val is not None else 0.0  # type: ignore[arg-type, invalid-argument-type]  # ty:ignore[invalid-argument-type]

            # Compute MAD: median(|X - median(X)|) * scale
            abs_dev = (col_data - self.median_[col]).abs()
            mad_val = abs_dev.median()
            mad_scaled = float(mad_val) * self.scale if mad_val is not None else 1.0  # type: ignore[arg-type, invalid-argument-type]  # ty:ignore[invalid-argument-type]
