...
class BaseConformityScorer(BaseScorer, metaclass=abc.ABCMeta):
    ...
    def _compute_assymetric_quantiles(conformity_scores: pl.DataFrame, coverage_rate: float) -> tuple[float, float]:
        ...
        if isinstance(conformity_scores, pl.DataFrame):
            conformity_scores = conformity_scores.to_numpy()

        # Check if array is empty
        if hasattr(conformity_scores, "size") and conformity_scores.size == 0:
            raise ValueError(
        ...
        upper_quantile: float = np.quantile(conformity_scores, 1 - coverage_rate / 2.0, method="higher")  # type: ignore[call-overload]
