...
def _(mo):
    ...
    n_points_slider = mo.ui.slider(start=50, stop=200, step=10, value=100, label="Number of points")
    noise_level_slider = mo.ui.slider(start=0.1, stop=2.0, step=0.1, value=0.5, label="Noise level")
    seed_slider = mo.ui.slider(start=0, stop=100, step=1, value=42, label="Random seed")
    ...
    for forecaster_name in scale_dep_df["Forecaster"].unique():
        for metric_name in scale_dep_df["Metric"].unique():
            subset = scale_dep_df.filter(
                (pl.col("Forecaster") == forecaster_name) & (pl.col("Metric") == metric_name)
            )

            fig_scale_dep.add_trace(
                go.Bar(
                    x=subset["Series"],
                    y=subset["Score"],
...
def _(
    ...
    mape = MeanAbsolutePercentageError(epsilon=epsilon_slider.value, aggregation_method="timewise")
    smape = SymmetricMeanAbsolutePercentageError(
        epsilon=epsilon_slider.value, aggregation_method="timewise"
    )
...
def _(mo):
    ...
    seasonality_slider = mo.ui.slider(start=1, stop=20, step=1, value=7, label="Seasonality period")
...
def _(
    ...
    mase = MeanAbsoluteScaledError(
        seasonality=seasonality_slider.value, aggregation_method="timewise"
    )
...
def _(mo):
    ...
    coverage_rate_slider = mo.ui.slider(
        start=0.5, stop=0.99, step=0.05, value=0.9, label="Coverage rate", show_value=True
    )
...
def _(
    ...
    interval_forecaster.fit(y_train, X_train, forecasting_horizon=10)

    # Generate intervals
    y_pred_interval = interval_forecaster.predict_interval(
        forecasting_horizon=10, coverage_rates=[coverage_rate_slider.value]
    )
    return (
        AbsoluteResidual,
...
def _(coverage_rate_slider, mo, pl, y_pred_interval, y_test):
    from yohou.metrics import CalibrationError, EmpiricalCoverage
    ...
    # Fit scorers on test data
    emp_coverage.fit(y_test)
    calib_error.fit(y_test)
...
def _(mo, pl, y_pred_interval, y_test):
    from yohou.metrics import IntervalScore, MeanIntervalWidth
    ...
    # Fit scorers on test data
    mean_width.fit(y_test)
    interval_score.fit(y_test)
        ...
        interval_metrics_rows.append(
            {
                ...
                "Interval Score": is_result.get_column(
                    [c for c in is_result.columns if series_name in c][0]
                )[0],
            }
        )
...
def _(
    ...
    y_pred_residual = forecaster_residual.predict_interval(
        forecasting_horizon=10, coverage_rates=[0.9]
    )
...
def _(mo):
    ...
    gamma_epsilon_slider = mo.ui.slider(
        start=1e-10, stop=1e-6, step=1e-10, value=1e-8, label="Gamma epsilon", show_value=True
    )
...
def _(
    ...
    from yohou.metrics.conformity import AbsoluteGammaResidual, GammaResidual
    ...
    # Fit
    forecaster_gamma.fit(y_train, X_train, forecasting_horizon=12)
    forecaster_abs_gamma.fit(y_train, X_train, forecasting_horizon=12)

    # Generate intervals
    y_pred_gamma = forecaster_gamma.predict_interval(forecasting_horizon=12, coverage_rates=[0.9])

    y_pred_abs_gamma = forecaster_abs_gamma.predict_interval(
        forecasting_horizon=12, coverage_rates=[0.9]
    )
...
@app.cell
def _(mo, result_agg):
    mo.ui.table(result_agg) if hasattr(result_agg, "columns") else mo.md(
        f"Scalar score: **{result_agg:.4f}**"
    )
    return
