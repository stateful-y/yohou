...
def _(mo):
    ...
    **Prerequisites:** Basic familiarity with sklearn's fit/predict API and classification concepts.
    """)
...
def _():
    ...
    from yohou.datasets import fetch_air_quality_classification, fetch_demand_classification
...
def _(mo):
    mo.md(r"""
    ## 9. Alternative Dataset: Demand Classification

    [`fetch_demand_classification`](/pages/api/generated/yohou.datasets._fetchers.fetch_demand_classification/)
    provides a binary electricity demand classification target derived from
    smart meter readings. It follows the same API as [`fetch_air_quality_classification`](/pages/api/generated/yohou.datasets._fetchers.fetch_air_quality_classification/)
    and works with [`ClassProbaReductionForecaster`](/pages/api/generated/yohou.class_proba.reduction.ClassProbaReductionForecaster/) without any code changes.
    """)


@app.cell
def _(fetch_demand_classification):
    demand_data = fetch_demand_classification()
    print(f"Classes: {demand_data.classes}")
    print(f"Dataset: {len(demand_data.y)} observations from {demand_data.y['time'].min()} to {demand_data.y['time'].max()}")
    demand_data.y.head(5)
    return (demand_data,)
