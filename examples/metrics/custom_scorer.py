...
def _(mo):
    ...
    **Prerequisites:** Familiarity with the built-in scorers
    ([Point Metrics](/examples/metrics/point_metrics/) · [Evaluate Forecast Accuracy](/pages/how-to/evaluate-forecast-accuracy/)).
    """)
...
def _(mo):
    ...
    Subclass `BasePointScorer` and implement `score`. The method validates
...
def _(mo):
    ...
    Custom scorers work directly with `GridSearchCV` and
    `RandomizedSearchCV`.
    """)
