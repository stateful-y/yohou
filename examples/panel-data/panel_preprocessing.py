...
__gallery__ = {
    "title": "How to Preprocess Panel Data",
    ...
    "api_references": ["RollingStatisticsTransformer", "SimpleTimeImputer", "dict_to_panel", "inspect_panel"],
}
...
def _():
    ...
    from yohou.datasets import fetch_pedestrian_counts, fetch_tourism_quarterly
...
def _(mo):
    mo.md(r"""
    The same structure applies to any panel dataset. Below we show
    [`fetch_pedestrian_counts`](/pages/api/generated/yohou.datasets._fetchers.fetch_pedestrian_counts/)
    (a high-frequency sensor panel with hourly Melbourne pedestrian counts
    across multiple sensor locations).
    """)


@app.cell
def _(fetch_pedestrian_counts, inspect_panel, mo):
    _bunch = fetch_pedestrian_counts()
    _selected_sensors = ["Bourke_Street_Mall__pedestrians", "Southern_Cross_Station__pedestrians", "Flagstaff_Station__pedestrians"]
    pedestrian = _bunch.frame.select("time", *[c for c in _bunch.frame.columns if any(s in c for s in ["Bourke", "Southern_Cross", "Flagstaff"])]).drop_nulls().head(500)
    _globals_p, panel_groups_p = inspect_panel(pedestrian)
    mo.md(
        f"**Shape**: {pedestrian.shape}\n\n"
        f"**Panel groups**: {list(panel_groups_p.keys())}\n\n"
        f"Hourly sensor data follows the same `GROUP__MEMBER` convention — "
        f"the same transforms from section 2 onwards apply unchanged."
    )
    return (pedestrian,)
...
def _(mo):
    mo.md(
        ...
        - [Panel Data Preprocessing](/pages/how-to/panel-data/) for the full guide
        - [Panel Data Forecasting Tutorial](/pages/tutorials/panel-data/) for related techniques
        """
    )
    return
