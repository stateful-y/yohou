...
Observation horizon { #observation-horizon .autolink }
:   The number of recent time steps a stateful component must retain in memory to
    produce output. A [`LagTransformer`](/pages/api/generated/yohou.preprocessing.LagTransformer/)
...
Composite method { #composite-method .autolink }
...
Point forecast { #point-forecast .autolink }
:   A single numeric prediction per timestep, produced by `predict()`.

Interval forecast { #interval-forecast .autolink }
...
Class-probability forecast { #class-probability-forecast .autolink }
...
Coverage rate { #coverage-rate .autolink }
...
Recursive prediction { #recursive-prediction .autolink }
...
Error accumulation { #error-accumulation .autolink }
...
:   The column in a prediction DataFrame that records the last observed timestamp
    ...
    plotting functions like [`plot_score_per_vintage`](/pages/api/generated/yohou.plotting.plot_score_per_vintage/).
...
:   A single position within the forecasting horizon (1-based). Step 1 is the
    ...
    scorers with `aggregation_method="stepwise"` and by [`plot_score_per_step`](/pages/api/generated/yohou.plotting.plot_score_per_step/).
...
Rolling evaluation { #rolling-evaluation .autolink }
...
Time column contract { #time-column-contract .autolink }
...
Panel data { #panel-data .autolink }
...
Group prefix { #group-prefix .autolink }
...
Panel strategy { #panel-strategy .autolink }
...
Exogenous features { #exogenous-features .autolink }
...
Known-future features { #known-future-features .autolink }
...
Step-indexed columns { #step-indexed-columns .autolink }
...
Tabularization { #tabularization .autolink }
...
Reduction strategy { #reduction-strategy .autolink }
...
Step feature alignment { #step-feature-alignment .autolink }
...
Target transformer { #target-transformer .autolink }
:   A [`BaseActualTransformer`](/pages/api/generated/yohou.base.BaseActualTransformer/)
...
Actual transformer { #actual-transformer .autolink }
:   A [`BaseActualTransformer`](/pages/api/generated/yohou.base.BaseActualTransformer/)
...
Forecast transformer { #forecast-transformer .autolink }
:   A [`BaseForecastTransformer`](/pages/api/generated/yohou.base.BaseForecastTransformer/)
...
:   Which frame shape a transformer consumes and produces, carried as a `kind` tag
    ...
    ([`BaseActualTransformer`](/pages/api/generated/yohou.base.BaseActualTransformer/))
    works on a single-axis frame with one `"time"` column; a forecast-kind transformer
    ([`BaseForecastTransformer`](/pages/api/generated/yohou.base.BaseForecastTransformer/))
...
Stateful transformer { #stateful-transformer .autolink }
:   An actual-kind [`BaseActualTransformer`](/pages/api/generated/yohou.base.BaseActualTransformer/)
...
Stateless transformer { #stateless-transformer .autolink }
...
:   Combining predictions from multiple forecasters to reduce variance. Implemented
    via the [`VotingPointForecaster`](/pages/api/generated/yohou.ensemble.VotingPointForecaster/), [`VotingIntervalForecaster`](/pages/api/generated/yohou.ensemble.VotingIntervalForecaster/), and
    [`VotingClassProbaForecaster`](/pages/api/generated/yohou.ensemble.VotingClassProbaForecaster/) classes.
    See [Ensemble Forecasting](ensemble-forecasting.md).

Forecaster composition { #forecaster-composition .autolink }
...
:   Separating a time series into additive components (trend, seasonality, residual) for
    separate modeling, implemented in [`DecompositionPipeline`](/pages/api/generated/yohou.compose.DecompositionPipeline/).
    See [Stationarity](stationarity.md).

Variance stabilization { #variance-stabilization .autolink }
...
Stationarity { #stationarity .autolink }
:   A time series is stationary when its statistical properties (mean, variance) do
    ...
    and Yohou provides transforms like [`SeasonalDifferencing`](/pages/api/generated/yohou.stationarity.SeasonalDifferencing/) and [`DecompositionPipeline`](/pages/api/generated/yohou.compose.DecompositionPipeline/)
    to achieve it before modeling. See [Stationarity](stationarity.md).

Conformal prediction { #conformal-prediction .autolink }
:   A distribution-free method for constructing prediction intervals with finite-sample
    coverage guarantees, implemented in [`SplitConformalForecaster`](/pages/api/generated/yohou.interval.SplitConformalForecaster/).
    See [Interval Forecasting](interval-forecasting.md).

Conformity score { #conformity-score .autolink }
...
Calibration set { #calibration-set .autolink }
...
Similarity measure { #similarity-measure .autolink }
...
Metadata routing { #metadata-routing .autolink }
...
Proper scoring rule { #proper-scoring-rule .autolink }
...
Aggregation method { #aggregation-method .autolink }
...
Forecast error { #forecast-error .autolink }
...
Scale-dependent metric { #scale-dependent-metric .autolink }
...
Temporal split { #temporal-split .autolink }
...
Expanding window splitter { #expanding-window-splitter .autolink }
...
Sliding window splitter { #sliding-window-splitter .autolink }
...
Concept drift { #concept-drift .autolink }
...
Time weighting { #time-weighting .autolink }
