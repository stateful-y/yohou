...
*[shared round]: The boosting iteration count a `validation="cv"` search chooses for one fitted estimator from the stopping metric averaged over the folds; every fold is scored at it and the refit trains that many iterations.
