...
both X_actual and y to the underlying sklearn estimator. This ensures that estimators
...
def reduction_data():
    ...
    return y[:40], y[40:], X_actual[:40], X_actual[40:]
...
class _SpyEstimator(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X_actual, y, **fit_params):
        self.fit_X_type = type(X_actual)
        self.fit_y_type = type(y)
        self.fit_X_columns = list(X_actual.columns) if hasattr(X_actual, "columns") else None
        if isinstance(y, pl.DataFrame):
            ...
            self.fit_y_columns = None
        self._inner.fit(X_actual, y)
        return self

    def predict(self, X_actual):
        _SpyEstimator.predict_calls.append({
            "X_type": type(X_actual),
            "X_columns": list(X_actual.columns) if hasattr(X_actual, "columns") else None,
        })
        return self._inner.predict(X_actual)
...
class TestFitReceivesDataFrames:
    ...
    def test_X_is_dataframe(self, reduction_data, strategy):
        """The estimator's fit() receives a Polars DataFrame for X_actual."""
        y_train, _, X_actual_train, _ = reduction_data
        spy = _SpyEstimator()
    ...
    def test_multi_target_multi_output_y_is_dataframe(self, multi_target_data):
        ...
        y_train, _, X_actual_train, _ = multi_target_data
        spy = _SpyEstimator()
...
class TestPredictReceivesDataFrames:
    ...
    @pytest.mark.parametrize("strategy", ["multi-output", "direct", "dir-rec"])
    def test_X_is_dataframe(self, reduction_data, strategy):
        """The estimator's predict() receives a Polars DataFrame for X_actual."""
        y_train, _, X_actual_train, X_actual_test = reduction_data
        spy = _SpyEstimator()
...
class TestFeatureNamesConsistency:
    ...
    def test_columns_include_lagged_target(self, reduction_data):
        ...
        forecaster.fit(y=y_train, X_actual=X_actual_train, forecasting_horizon=2)
    ...
    def test_panel_fit_predict_columns_match(self, panel_data, strategy):
        ...
        y_train, _, X_actual_train, X_actual_test = panel_data
        spy = _SpyEstimator()
        _SpyEstimator.reset_predict_calls()
        forecaster = PointReductionForecaster(estimator=spy, reduction_strategy=strategy)
        forecaster.fit(y=y_train, X_actual=X_actual_train, forecasting_horizon=3)
...
class TestPanelDataFramePipeline:
    ...
    def test_X_is_dataframe_at_fit(self, panel_data, strategy):
        """Panel: estimator's fit() receives a Polars DataFrame for X_actual."""
        y_train, _, X_actual_train, _ = panel_data
        spy = _SpyEstimator()
        forecaster = PointReductionForecaster(estimator=spy, reduction_strategy=strategy)
        forecaster.fit(y=y_train, X_actual=X_actual_train, forecasting_horizon=3)
    ...
    def test_X_is_dataframe_at_predict(self, panel_data, strategy):
        """Panel: estimator's predict() receives a Polars DataFrame for X_actual."""
        y_train, _, X_actual_train, X_actual_test = panel_data
        spy = _SpyEstimator()
        _SpyEstimator.reset_predict_calls()
        forecaster = PointReductionForecaster(estimator=spy, reduction_strategy=strategy)
...
class TestPredictionCorrectness:
    ...
    def test_multi_target_shape_and_columns(self, multi_target_data):
        ...
        y_train, _, X_actual_train, X_actual_test = multi_target_data
        forecaster = PointReductionForecaster(estimator=LinearRegression())
        forecaster.fit(y=y_train, X_actual=X_actual_train, forecasting_horizon=3)
        y_pred = forecaster.predict(forecasting_horizon=3)
