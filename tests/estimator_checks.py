...
def check_fit_sets_attributes(transformer, X, y=None):
    ...
    assert hasattr(transformer_clone, "n_features_in_"), \
        "fit() must set n_features_in_ attribute"
...
def check_observation_horizon_after_fit(transformer, X, y=None):
    ...
    assert isinstance(horizon, int), \
        f"observation_horizon must be int, got {type(horizon)}"
    assert horizon >= 0, \
        f"observation_horizon must be non-negative, got {horizon}"
...
def check_transform_output_structure(transformer, X, y=None):
    ...
    assert "time" in X_trans.columns, \
        "transform() output must contain 'time' column"
...
def check_panel_data_support(transformer, X_panel, y=None):
    ...
    try:
        ...
        assert isinstance(X_trans, pl.DataFrame), \
            "Panel data transform must return DataFrame"
        assert "time" in X_trans.columns, \
            "Panel data transform must preserve 'time' column"
...
def check_clone_preserves_params(transformer):
    ...
    assert transformer_clone is not transformer, \
...
def check_fit_idempotent(transformer, X, y=None):
    ...
    for attr in ["feature_names_in_", "n_features_in_", "_observation_horizon"]:
        if hasattr(transformer1, attr):
            ...
            assert val1 == val2, \
                f"Attribute '{attr}' differs after double fit: {val1} vs {val2}"
...
def _yield_yohou_transformer_checks(
    ...
    yield "check_transform_output_structure", check_transform_output_structure, {"X": X_train, "y": y}
    ...
    if tags.get("stateful", True):
        yield "check_observation_horizon_not_fitted", check_observation_horizon_not_fitted, {"X": X_train}
        yield "check_observation_horizon_after_fit", check_observation_horizon_after_fit, {"X": X_train, "y": y}
        yield "check_reset_updates_memory", check_reset_updates_memory, {"X": X_train, "y": y}
        yield "check_update_concatenates_memory", check_update_concatenates_memory, {"X": X_train, "y": y}
        yield "check_update_transform_equivalence", check_update_transform_equivalence, {"X": X_train, "y": y}
        yield "check_insufficient_data_raises", check_insufficient_data_raises, {"X": X_train, "y": y}
        yield "check_memory_bounded", check_memory_bounded, {"X": X_train, "y": y}
    else:
        # Stateless checks
        yield "check_transformers_unfitted_stateless", check_transformers_unfitted_stateless, {"X": X_test}
    
    # Invertible transformer checks
    if tags.get("invertible", False):
        yield "check_inverse_transform_identity", check_inverse_transform_identity, {"X": X_test, "y": y}
        yield "check_inverse_transform_round_trip", check_inverse_transform_round_trip, {"X": X_test, "y": y}
    
    # sklearn compatibility checks
    yield "check_transformer_preserve_dtypes", check_transformer_preserve_dtypes, {"X": X_train, "y": y}
...
def check_fit_sets_forecaster_attributes(forecaster, y, X_ante=None, X_post=None, forecasting_horizon=3):
    ...
    forecaster_clone.fit(y, X_ante, X_post, forecasting_horizon=forecasting_horizon)
    ...
    assert hasattr(forecaster_clone, "interval_"), \
        "fit() must set interval_ attribute (timedelta)"
    ...
    assert hasattr(forecaster_clone, "_y_observed"), \
        "fit() must set _y_observed buffer"
    assert hasattr(forecaster_clone, "_X_t_observed"), \
        "fit() must set _X_t_observed buffer"
    
    if X_ante is not None:
        assert hasattr(forecaster_clone, "_X_ante_observed"), \
            "fit() must set _X_ante_observed buffer when X_ante provided"
...
def check_forecaster_not_fitted_error(forecaster, y, X_ante=None, X_post=None):
...
def check_predict_time_columns(forecaster, y_test, X_ante_test=None, X_post_test=None):
    ...
    X_ante_test : pl.DataFrame, optional
        Test ex-ante features (not used - stored in _X_ante_observed during fit)
    ...
    Notes
    -----
    X_ante is not passed to predict() because ex-ante features (known in advance)
    are stored during fit() and used automatically. Only X_post can be provided
    at predict time since those are "observed after" features.
    """
    forecasting_horizon = min(3, len(y_test))
    y_pred = forecaster.predict(forecasting_horizon=forecasting_horizon, X_post=X_post_test)
    
    assert "observed_time" in y_pred.columns, \
        "Predictions must have 'observed_time' column"
    assert "time" in y_pred.columns, \
        "Predictions must have 'time' column"
    ...
    assert y_pred["observed_time"].dtype == pl.Datetime, \
        "observed_time must be Datetime dtype"
    assert y_pred["time"].dtype == pl.Datetime, \
        "time must be Datetime dtype"


def check_update_extends_observations(forecaster, y_train, y_update, X_ante_train=None, X_ante_update=None, X_post_train=None, X_post_update=None):
    ...
    forecaster.update(y_update, X_ante_update, X_post_update)
...
def check_reset_replaces_observations(forecaster, y_train, y_reset, X_ante_train=None, X_ante_reset=None, X_post_train=None, X_post_reset=None):
    ...
    """
    # Reset to new data
    forecaster.reset(y_reset, X_ante_reset, X_post_reset)
...
def check_reset_propagates_to_transformers(forecaster, y_train, y_reset, X_ante_train=None, X_ante_reset=None, X_post_train=None, X_post_reset=None):
    ...
    if forecaster.target_transformer is not None and hasattr(forecaster.target_transformer_, "_X_observed"):
        original_target_time = forecaster.target_transformer_._X_observed["time"].max()
    else:
        original_target_time = None
    
    if forecaster.feature_transformer is not None and hasattr(forecaster.feature_transformer_, "_X_observed"):
    ...
    if original_feature_time is not None:
        ...
        # Feature transformer gets combined X_ante/X_post, so check it was updated
...
def check_forecasting_horizon_validation(forecaster, y, X_ante=None, X_post=None):
    ...
    # Test horizon = 0
    try:
        forecaster_clone.fit(y, X_ante, X_post, forecasting_horizon=0)
        raise AssertionError(
    ...
    forecaster_clone = clone(forecaster)
    try:
        forecaster_clone.fit(y, X_ante, X_post, forecasting_horizon=-1)
        raise AssertionError(
            f"{forecaster_clone.__class__.__name__} should raise ValueError "
...
def check_prediction_types_property(forecaster):
    ...
    assert len(pred_types) > 0, \
        "prediction_types should not be empty"
...
def check_clone_preserves_forecaster_params(forecaster):
    ...
    for key in original_params:
        ...
        # For estimator instances, check type and params
        ...
        # For other values, direct comparison
            ...
            assert orig_val == cloned_val, \
                f"Parameter {key}: {cloned_val} != {orig_val}"
    
    # Check they are different objects
    assert forecaster_clone is not forecaster, \
        "clone() should create new instance"
...
def check_point_prediction_structure(forecaster, y_test, X_ante_test=None, X_post_test=None):
...
def check_interval_prediction_columns(forecaster, y_test, X_ante_test=None, X_post_test=None):
    ...
    # Get target columns (excluding time and interval columns)
...
def check_interval_bounds(forecaster, y_test, X_ante_test=None, X_post_test=None):
...
def check_coverage_rates_parameter(forecaster):
    ...
    assert len(coverage_rates) > 0, \
        "coverage_rates should not be empty"
    
    for rate in coverage_rates:
        assert isinstance(rate, (int, float)), \
            f"Each coverage rate should be numeric, got {type(rate)} for {rate}"
        assert 0 < rate < 1, \
            f"Coverage rates should be in (0, 1), got {rate}"
...
def check_estimator_parameter(forecaster):
    ...
    assert hasattr(forecaster, "estimator"), \
        "Reduction forecaster must have 'estimator' parameter"
...
def _yield_yohou_forecaster_checks(
    ...
    yield "check_fit_sets_forecaster_attributes", check_fit_sets_forecaster_attributes, {
        "y": y_train, "X_ante": X_ante_train, "X_post": X_post_train, "forecasting_horizon": 3
    }
    yield "check_forecaster_not_fitted_error", check_forecaster_not_fitted_error, {
        "y": y_train, "X_ante": X_ante_train, "X_post": X_post_train
    }
    yield "check_predict_time_columns", check_predict_time_columns, {
        "y_test": y_test, "X_ante_test": X_ante_test, "X_post_test": X_post_test
    }
    yield "check_forecasting_horizon_validation", check_forecasting_horizon_validation, {
        "y": y_train, "X_ante": X_ante_train, "X_post": X_post_train
    }
    ...
    if len(y_test) >= 5:
        ...
        yield "check_update_extends_observations", check_update_extends_observations, {
            "y_train": y_train, "y_update": y_update,
            "X_ante_train": X_ante_train, "X_ante_update": X_ante_update,
            "X_post_train": X_post_train, "X_post_update": X_post_update,
        }
        yield "check_reset_replaces_observations", check_reset_replaces_observations, {
            "y_train": y_train, "y_reset": y_reset,
            "X_ante_train": X_ante_train, "X_ante_reset": X_ante_reset,
            "X_post_train": X_post_train, "X_post_reset": X_post_reset,
        }
    
    # Transformer composition checks
    if tags.get("uses_transformers", False):
        if len(y_test) >= 5:
            yield "check_reset_propagates_to_transformers", check_reset_propagates_to_transformers, {
                "y_train": y_train, "y_reset": y_reset,
                "X_ante_train": X_ante_train, "X_ante_reset": X_ante_reset,
                "X_post_train": X_post_train, "X_post_reset": X_post_reset,
            }
    ...
    if tags.get("forecaster_type") == "point":
        yield "check_point_prediction_structure", check_point_prediction_structure, {
            "y_test": y_test, "X_ante_test": X_ante_test, "X_post_test": X_post_test
        }
    ...
    if tags.get("forecaster_type") == "interval":
        yield "check_interval_prediction_columns", check_interval_prediction_columns, {
            "y_test": y_test, "X_ante_test": X_ante_test, "X_post_test": X_post_test
        }
        yield "check_interval_bounds", check_interval_bounds, {
            "y_test": y_test, "X_ante_test": X_ante_test, "X_post_test": X_post_test
        }
