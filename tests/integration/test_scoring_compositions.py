...
def test_point_scorer_on_forecaster_composition(linear_series, forecaster, scorer):
...
def test_point_scorer_perfect_prediction(constant_series, scorer):
...
def test_point_scorer_on_constant_data(constant_series, scorer):
...
@pytest.mark.integration
def test_mase_requires_fit(linear_series):
...
@pytest.mark.integration
def test_rmsse_requires_fit(linear_series):
...
@pytest.mark.integration
def test_mase_equals_one_when_matches_naive(linear_series):
...
@pytest.mark.integration
def test_rmsse_equals_one_when_matches_naive(linear_series):
...
def test_interval_scorer_on_conformal(linear_series, scorer):
    ...
    forecaster = SplitConformalForecaster(base_forecaster, conformity_scorer=AbsoluteResidual(), calibration_size=20)
...
@pytest.mark.integration
def test_calibration_error_requires_multiple_coverage_rates(linear_series):
...
@pytest.mark.integration
def test_empirical_coverage_returns_value_in_0_1(linear_series):
...
@pytest.mark.integration
def test_interval_score_is_nonnegative(linear_series):
...
@pytest.mark.integration
def test_mean_interval_width_is_positive(linear_series):
...
def test_interval_scorer_column_subselection(linear_series, coverage_rates):
...
def test_point_scorer_aggregation_on_panel(linear_series, panel_analytical, aggregation_method):
...
def test_scorer_aggregation_output_shapes(linear_series, _scorer_cls, aggregation_method):
...
@pytest.mark.integration
def test_scorer_with_exponential_decay_weight(constant_series):
...
@pytest.mark.integration
def test_scorer_with_linear_decay_weight(linear_series):
...
@pytest.mark.integration
def test_weighted_mae_analytic_verification(constant_series):
    ...
    assert np.abs(score - expected) < 1e-6, f"Weighted MAE should match analytical: expected={expected}, got={score}"
...
def test_time_weight_with_forecaster_compositions(linear_series, forecaster):
...
@pytest.mark.integration
def test_time_weight_verification_early_vs_late_errors(constant_series):
...
@pytest.mark.integration
def test_mape_with_zero_values_handling(linear_series):
...
@pytest.mark.integration
def test_interval_coverage_on_wide_intervals(linear_series):
...
@pytest.mark.integration
def test_conformity_scorer_residual(linear_series):
...
@pytest.mark.integration
def test_scorer_on_single_timestep_prediction(linear_series):
