...
def test_pipelineobservation_horizon_two_lags(linear_series):
...
@pytest.mark.integration
def test_pipelineobservation_horizon_lag_and_seasonal_diff(linear_series):
...
@pytest.mark.integration
def test_pipelineobservation_horizon_three_stages(linear_series):
...
@pytest.mark.integration
def test_pipelineobservation_horizon_rolling_stats(linear_series):
...
@pytest.mark.integration
def test_pipelineobservation_horizon_with_stateless_transformer(linear_series):
...
@pytest.mark.integration
def test_pipeline_reduction_linear_ar1_equivalence(linear_series, analytical_forecast):
...
@pytest.mark.integration
def test_pipeline_reduction_ar1_process(ar1_series, analytical_forecast):
...
@pytest.mark.integration
def test_feature_union_two_lags(linear_series):
    ...
    # Check feature names: should have lag1__value_lag_1 and lag2__value_lag_2
    feature_cols = [c for c in X_transformed.columns if c != "time"]
    assert "lag1__value_lag_1" in feature_cols
    assert "lag2__value_lag_2" in feature_cols


@pytest.mark.integration
def test_feature_union_different_transformers(linear_series):
    ...
    feature_cols = [c for c in X_transformed.columns if c != "time"]
    assert any("lag__value_lag_1" in c for c in feature_cols)
    assert any("lag__value_lag_2" in c for c in feature_cols)
    assert any("roll__value_mean" in c for c in feature_cols)
    assert any("roll__value_std" in c for c in feature_cols)


@pytest.mark.integration
def test_feature_union_with_reduction_ar2_equivalence(linear_series, analytical_forecast):
...
@pytest.mark.integration
def test_column_transformer_independent_transformations(linear_series):
    ...
    feature_cols = [c for c in X_transformed.columns if c != "time"]
    assert any("col1_lag__col1_lag_1" in c for c in feature_cols)
    assert any("col2_lag__col2_lag_2" in c for c in feature_cols)
    assert any("col3_lag__col3_lag_3" in c for c in feature_cols)


@pytest.mark.integration
def test_column_transformer_remainder_passthrough(linear_series):
    ...
    assert "remainder__col3" in X_transformed.columns


@pytest.mark.integration
def test_column_transformer_remainder_drop(linear_series):
...
@pytest.mark.integration
def test_column_forecaster_independent_slopes(linear_series, analytical_forecast):
    ...
    col_forecaster = ColumnForecaster(
        [
            ("col1", PointReductionForecaster(LinearRegression(), feature_transformer=LagTransformer([1])), ["col1"]),
            ("col2", PointReductionForecaster(LinearRegression(), feature_transformer=LagTransformer([1])), ["col2"]),
            ("col3", PointReductionForecaster(LinearRegression(), feature_transformer=LagTransformer([1])), ["col3"]),
        ],
        remainder="drop",
    )
...
@pytest.mark.integration
def test_column_forecaster_with_remainder_passthrough(linear_series):
...
@pytest.mark.integration
def test_column_forecaster_different_forecaster_types(linear_series, ar1_series):
...
@pytest.mark.integration
def test_decomposition_pipeline_trend_constant_residual(linear_series):
...
@pytest.mark.integration
def test_decomposition_pipeline_trend_seasonal(trend_plus_seasonal, analytical_forecast):
...
@pytest.mark.integration
def test_decomposition_pipeline_three_components(linear_series):
...
def test_forecasted_feature_linear_dependency(linear_series, analytical_forecast, strategy):
...
def test_forecasted_feature_different_split_ratios(linear_series, split_ratio):
...
@pytest.mark.integration
def test_forecasted_feature_strategy_actual_uses_provided_X(linear_series):
...
@pytest.mark.integration
def test_deep_nesting_column_forecaster_with_pipelines(linear_series, analytical_forecast):
...
@pytest.mark.integration
def test_deep_nesting_decomposition_in_column_forecaster(linear_series):
...
@pytest.mark.integration
def test_observe_propagation_decomposition_pipeline(linear_series, analytical_forecast):
...
@pytest.mark.integration
def test_observe_propagation_column_forecaster(linear_series):
...
@pytest.mark.integration
def test_observe_propagation_feature_pipeline(linear_series):
...
@pytest.mark.integration
def test_rewind_propagation_decomposition_pipeline(linear_series):
...
@pytest.mark.integration
def test_rewind_propagation_column_forecaster(linear_series):
...
@pytest.mark.integration
def test_rewind_propagation_feature_pipeline(linear_series):
