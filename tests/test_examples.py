...
_NOTEBOOK_MARKS: dict[str, list[pytest.MarkDecorator]] = {
    ...
    "catboost_forecasting.py": [
        pytest.mark.xfail(reason="CatBoost multidimensional target incompatibility", strict=True),
    ],
    "catboost_multiquantile.py": [
        pytest.mark.xfail(reason="CatBoost multidimensional target incompatibility", strict=True),
    ],
    "forecasted_feature_advanced.py": [
        pytest.mark.xfail(reason="ForecastedFeatureForecaster rewind strategy pre-existing bug", strict=True),
    ],
}
