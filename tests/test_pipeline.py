...
def test_featureunion_horizontal_concat(dummy_transformers, time_series_factory):
    ...
    pytest.skip(
        "FeatureUnion doesn't add transformer name prefixes yet - causes duplicate column names"
    )
...
def test_columntransformer_column_selection(time_series_factory):
    ...
    assert len(feature_cols) >= 2, (
        f"Expected at least 2 feature columns, got {len(feature_cols)}: {feature_cols}"
    )
