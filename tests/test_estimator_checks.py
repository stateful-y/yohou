...
def check_update_reset_contract(estimator, time_series_factory):
    ...
    # update should work
    assert hasattr(estimator_clone, "update"), \
        "Transformer must have update() method"
    estimator_clone.update(X_new)
    
    # reset should work
    assert hasattr(estimator_clone, "reset"), \
        "Transformer must have reset() method"
...
def check_polars_dataframe_io(estimator, time_series_factory):
    ...
    assert isinstance(X, pl.DataFrame), \
        "Test data should be polars DataFrame"
...
def test_all_transformers_clonable(transformer_registry):
    ...
    for name, config in transformer_registry.items():
        ...
        assert transformer_clone is not transformer, \
            f"{name}: clone should create new object"
...
def test_all_transformers_fit_transform(transformer_registry, time_series_factory):
    ...
    for name, config in transformer_registry.items():
        ...
        assert isinstance(X_trans, pl.DataFrame), \
            f"{name}: fit_transform should return DataFrame"
        assert "time" in X_trans.columns, \
            f"{name}: fit_transform output should have 'time' column"
...
def test_all_transformers_get_feature_names_out(transformer_registry, time_series_factory):
    ...
    for name, config in transformer_registry.items():
        ...
        assert feature_names is not None, \
            f"{name}: get_feature_names_out should not return None"
        assert len(feature_names) > 0, \
            f"{name}: get_feature_names_out should return non-empty list"
