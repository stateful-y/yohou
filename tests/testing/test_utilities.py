...
def test_y_X_factory_panel(y_X_factory):
...
def test_X_factory_alias(X_factory):
...
def test_pattern_factory_linear_trend(pattern_factory):
...
def test_pattern_factory_seasonal(pattern_factory):
    ...
    values = df["target"].to_list()
    # At t=0 and t=12, sin(0) and sin(2π) ≈ 0
    assert abs(values[0]) < 0.1
    assert abs(values[12]) < 0.1


def test_pattern_factory_all_patterns(pattern_factory):
...
def test_assert_forecaster_output_valid_basic():
...
def test_assert_forecaster_output_valid_with_columns():
...
def test_assert_continuous_time():
...
def test_assert_transformer_output_valid(X_factory):
...
def test_integration_pattern_factory_with_forecaster(pattern_factory):
