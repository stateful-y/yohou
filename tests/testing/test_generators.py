...
def test_yield_yohou_transformer_checks(y_X_factory):
    ...
    for check_name, check_func, check_kwargs in checks:
        ...
        assert isinstance(check_kwargs, dict), (
            f"Check kwargs should be dict, got {type(check_kwargs)}"
        )


def test_yield_yohou_forecaster_checks(y_X_factory):
...
def test_yield_yohou_transformer_checks_filters_by_tags(y_X_factory):
...
def test_yield_yohou_forecaster_checks_point_vs_interval(y_X_factory):
    """Test _yield_yohou_forecaster_checks handles different forecaster types."""
    y, X = y_X_factory(length=50, n_targets=1, n_features=2, seed=42)

    # Point forecaster
    point_forecaster = SeasonalNaive(seasonality=12)
    point_forecaster.fit(y[:40], X[:40], forecasting_horizon=3)
    ...
    assert not any(
        "interval" in name.lower() and "coverage" in name.lower() for name in check_names
    )


def test_yield_yohou_transformer_checks_expected_failures(y_X_factory):
