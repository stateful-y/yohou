...
def test_fourier_model_panel_behaviors(panel_time_series_factory, model_panel):
    ...
    if model_panel:
        ...
        assert isinstance(forecaster.estimator_, dict), "model_panel=True should store dict of estimators"
        ...
        for group_estimator in forecaster.estimator_.values():
            assert isinstance(group_estimator, Pipeline), "Each group estimator should be a Pipeline"
    else:
        # Pooled strategy: estimator_ should be single Pipeline object
        assert isinstance(forecaster.estimator_, Pipeline), "model_panel=False should store single Pipeline"
...
def test_fourier_model_panel_prediction_differences(panel_time_series_factory):
    ...
    for col in ["group0__series_0", "group1__series_0", "group2__series_0"]:
        ...
        assert mean_rel_diff > 0.01, f"Predictions for {col} should differ between strategies (got {mean_rel_diff:.4f})"
