...
class SimpleTransformer(BaseTransformer):
    ...
    def transform(self, X):
        return X.select([
            pl.col("time"),
            (cs.numeric() & ~cs.by_name("time")) + self.add_constant
        ])
...
class StatelessTransformer(BaseTransformer):
    ...
    def transform(self, X):
        return X.select([
            pl.col("time"),
            (cs.numeric() & ~cs.by_name("time")) * self.multiplier
        ])
...
class InvertibleTransformer(BaseTransformer):
    ...
    def transform(self, X):
        return X.select([
            pl.col("time"),
            (cs.numeric() & ~cs.by_name("time")) + self.offset
        ])
...
def panel_time_series_factory():
    ...
    def _make(length=50, n_series=3, n_global=2, seed=42):
        ...
        panel_df = pl.DataFrame(
            {"panel": pl.DataFrame(series_data)},
            schema={"panel": pl.Struct(schema)}
        )
...
def transformer_registry():
    ...
    return {
        ...
        "LagTransformer": {
            ...
            "expected_failed_checks": ["check_inverse_transform_identity", "check_inverse_transform_round_trip"],
...
@pytest.fixture
def y_X_factory():
    """Factory for generating (y, X_ante, X_post) tuples.
    ...
    def _factory(length=100, n_y_features=2, n_X_ante_features=3, 
                 n_X_post_features=2, seed=42, panel=False):
