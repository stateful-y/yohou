...
def test_rmsse_tags():
    ...
    assert tags.scorer_tags.requires_calibration is True, "RootMeanSquaredScaledError should require calibration (fit)"
...
def test_point_scorers_comparison(y_X_factory):
    ...
    assert mse_score == pytest.approx(np.mean(errors**2), abs=1e-5), "MeanSquaredError should match mean squared error"
...
def test_rmsse_per_step():
    ...
    assert (result["rmsse"] >= 0).all(), "All componentwise RootMeanSquaredScaledError values should be non-negative"
