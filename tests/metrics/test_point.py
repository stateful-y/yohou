...
@pytest.fixture
def y_X_factory():
    """Factory for generating time series data for testing."""
...
def test_rmse_equals_sqrt_mse(y_true_y_pred):
...
def test_rmse_units_match_target(y_true_y_pred):
...
def test_rmse_multiple_columns(y_X_factory):
    """RMSE should work with multiple target columns."""
    y, _ = y_X_factory(length=50, n_targets=3, n_features=0, seed=42)

    # Create predictions with small errors
    y_pred = y.clone()
    y_pred = y_pred.with_columns(
        [pl.col(col) + np.random.randn(50) * 2 for col in y_pred.columns if col != "time"]
    )
...
# ============================================================================
# RootMeanSquaredScaledError Tests
# ============================================================================


def test_rmsse_requires_fit():
...
def test_rmsse_requires_training_data():
...
def test_rmsse_seasonality_too_large_error():
...
def test_rmsse_scaling_factors():
    ...
    assert hasattr(rmsse, "scales_"), (
        "RootMeanSquaredScaledError should have scales_ attribute after fit"
    )
...
def test_rmsse_different_seasonalities(y_X_factory):
    ...
    for seasonality in [1, 7, 12, 24]:
        ...
        assert score >= 0, (
            f"RootMeanSquaredScaledError with seasonality={seasonality} should be non-negative"
        )


def test_rmsse_perfect_prediction_nonzero():
...
def test_rmsse_vs_naive_baseline():
...
def test_all_point_scorers_stateless_except_rmsse():
...
def test_point_scorers_comparison(y_X_factory):
    ...
    y_pred = y_pred.with_columns(
        [(pl.col(col) + errors).alias(col) for col in y_pred.columns if col != "time"]
    )
    ...
    assert np.isclose(rmse_score, np.sqrt(mse_score)), "RMSE should equal sqrt(MeanSquaredError)"
    assert mae_score == pytest.approx(np.mean(np.abs(errors)), abs=1e-5), (
        "MAE should match mean absolute error"
    )
...
def test_mae_per_step(y_true_y_pred):
    ...
    assert isinstance(result, pl.DataFrame), (
        "aggregation_method=['componentwise'] should return DataFrame"
    )
    ...
    assert np.allclose(result["mae"].to_numpy(), expected_errors, atol=1e-5), (
        "Per-step MAE values incorrect"
    )
...
def test_mse_per_step(y_true_y_pred):
...
def test_rmse_per_step(y_true_y_pred):
    ...
    assert np.allclose(result["rmse"].to_numpy(), expected_errors, atol=1e-5), (
        "Per-step RMSE values incorrect"
    )
...
def test_per_step_multiple_columns(y_X_factory):
    """componentwise aggregate should average across multiple columns."""
    y, _ = y_X_factory(length=20, n_targets=2, n_features=0, seed=42)
    ...
    y_pred = y_test.clone()
    y_pred = y_pred.with_columns(
        [(pl.col(col) + 1.0).alias(col) for col in y_pred.columns if col != "time"]
    )
    y_pred = y_pred.with_columns(observed_time=pl.lit(y_train["time"][-1]))
...
def test_aggregate_parameter_validation():
