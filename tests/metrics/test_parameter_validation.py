...
def test_panel_group_names_must_be_list_or_none(y_X_factory):
    ...
    with pytest.raises(
        ValueError, match=r"panel_group_names.*must be an instance of 'list' or None"
    ):
        scorer.fit(y)


def test_panel_group_names_elements_must_be_strings(y_X_factory):
...
def test_panel_group_names_cannot_be_empty_list(y_X_factory):
...
def test_panel_group_names_validated_against_data(panel_data_factory):
...
def test_valid_panel_group_names_accepted(panel_data_factory):
...
def test_component_names_must_be_list_or_none(y_X_factory):
...
def test_component_names_elements_must_be_strings(y_X_factory):
...
def test_component_names_cannot_be_empty_list(y_X_factory):
...
def test_component_names_validated_against_global_data(y_X_factory):
...
def test_component_names_validated_against_panel_data(panel_data_factory):
...
def test_valid_component_names_accepted_global(y_X_factory):
...
def test_valid_component_names_accepted_panel(panel_data_factory):
...
def test_coverage_rates_must_be_list_or_none(y_X_factory):
    """coverage_rates must be a list or None."""
    y, _ = y_X_factory(length=50, n_targets=1, seed=42)

    scorer = EmpiricalCoverage(coverage_rates="invalid")
    with pytest.raises(
        ValueError, match=r"coverage_rates.*must be an instance of 'list' or None"
    ):
        scorer.fit(y)


def test_coverage_rates_elements_must_be_numeric(y_X_factory):
...
def test_coverage_rates_cannot_be_empty_list(y_X_factory):
...
def test_coverage_rates_must_be_between_zero_and_one(y_X_factory):
    ...
    with pytest.raises(
        ValueError, match="All coverage_rates must be between 0 and 1.*got 0.0"
    ):
        scorer.fit(y)
    ...
    with pytest.raises(
        ValueError, match="All coverage_rates must be between 0 and 1.*got 1.0"
    ):
        scorer.fit(y)
    ...
    with pytest.raises(
        ValueError, match="All coverage_rates must be between 0 and 1.*got -0.1"
    ):
        scorer.fit(y)
    ...
    with pytest.raises(
        ValueError, match="All coverage_rates must be between 0 and 1.*got 1.5"
    ):
        scorer.fit(y)


def test_valid_coverage_rates_accepted(y_X_factory):
...
def test_type_validation_without_training_data():
    ...
    scorer = MeanAbsoluteError(component_names="invalid")
    with pytest.raises(
        ValueError, match=r"component_names.*must be an instance of 'list' or None"
    ):
        scorer.fit(None)
...
def test_data_validation_requires_training_data():
...
def test_combined_validation_all_parameters_validated(panel_data_factory):
...
def test_combined_validation_valid_parameters_all_accepted(panel_data_factory):
...
def test_panel_group_names_edge_case_with_mixed_types(y_X_factory):
...
def test_panel_group_names_edge_case_multiple_valid_groups(panel_data_factory):
...
def test_panel_group_names_edge_case_all_groups(panel_data_factory):
...
def test_panel_group_names_edge_case_single_missing(panel_data_factory):
...
def test_panel_group_names_edge_case_on_global_data_ignored(y_X_factory):
    ...
    scorer = MeanAbsoluteError(panel_group_names=["store1"])
    with pytest.raises(
        ValueError, match="panel_group_names specified but data contains no panel groups"
    ):
        scorer.fit(y)


def test_component_names_edge_case_with_mixed_types(y_X_factory):
...
def test_component_names_edge_case_multiple_valid_components(y_X_factory):
...
def test_component_names_edge_case_all_components(y_X_factory):
...
def test_component_names_edge_case_single_missing_global(y_X_factory):
...
def test_component_names_edge_case_partial_match_panel(panel_data_factory):
...
def test_component_names_edge_case_unprefixed_in_panel_data(panel_data_factory):
...
def test_coverage_rates_edge_case_with_mixed_numeric_types(y_X_factory):
...
def test_coverage_rates_edge_case_boundary_values_excluded(y_X_factory):
...
def test_coverage_rates_edge_case_very_small_value(y_X_factory):
...
def test_coverage_rates_edge_case_very_large_value(y_X_factory):
...
def test_coverage_rates_edge_case_multiple_invalid(y_X_factory):
...
def test_coverage_rates_edge_case_none_accepted(y_X_factory):
...
def test_validation_ordering_point_scorer_validation_order(panel_data_factory):
...
def test_validation_ordering_interval_scorer_validation_order(panel_data_factory):
...
def test_validation_with_score_method_validated_panel_groups_filter_in_score(panel_data_factory):
...
def test_validation_with_score_method_validated_components_filter_in_score(y_X_factory):
...
def test_validation_with_score_method_validated_coverage_rates_filter_in_score(y_X_factory):
